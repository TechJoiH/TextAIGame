...
namespace Logic.Intent
{
    ...
    public class SemanticMatcher
    {
        ...
        private static readonly Dictionary<string, string[]> SynonymDict = new Dictionary<string, string[]>
        {
            ...
            // 观察类同义词
            { "观察", new[] { "看", "瞧", "瞅", "盯", "望", "视", "端详", "打量", "审视", "扫视", "环顾", "张望", "窥探", "侦查", "搜寻" } },
            // 休息类同义词
    ...
    public class IntentRecognizer
    {
        ...
        private IntentRecognizer()
        {
            ...
            _targetExtractor = new Regex(
                @"(?:对|向|往|朝|去|把|攻击|查看|观察|使用|施展|释放)\s*[""「『]?(\S{1,10})[""」』]?|(\S{2,6})(?:身上|那边|那里|处|旁)",
        ...
        private List<IntentRule> InitializeRules()
        {
            return new List<IntentRule>
            {
                ...
                new IntentRule
                {
                    ...
                    keywords = new[] { "看", "观察", "查看", "注视", "审视", "端详", "打量", "望", "环顾", "瞧", "瞅" },
                    semanticPatterns = new[] { "仔细观察", "看看情况", "打量一番", "观望" },
                    regexPattern = @"(看|观察|查看|注视|审视|端详|打量|环顾|瞧|瞅).{0,6}",
                    priority = 5
                },
                new IntentRule
                {
